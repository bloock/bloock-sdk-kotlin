    ...
    fun testReadMeContainsVersionThatMatches() {
        ...
        BufferedReader(
            ...
            for (line in reader.lines()) {
                if (line.contains(BloockClient.VERSION)) {
                    mentioningLines.add(line)
                }
            }
            val record = java.lang.String.format(
                "Expected %d mentions of the version in the Readme, but found %d:%n",
                expectedMentionsOfVersion,
                mentioningLines.size
            )
            assertSame(expectedMentionsOfVersion, mentioningLines.size, record)
        }
    }
        ...
        BufferedReader(
            ...
            fail(
                java.lang.String.format(
                    "Expected the BloockClient.VERSION (%s) to match up with the one listed in the "
                            + "build.gradle.kts file. It wasn't found.",
                    BloockClient.VERSION
                )
            )
        }
    }
